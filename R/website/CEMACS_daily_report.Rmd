...
# Color code the points
#ii <- cut(co2_polar, breaks = seq(min(co2_polar), max(co2_polar), 
#                                  by = round((max(co2_polar) - min(co2_polar))/6,2)))
#colors <- colorRampPalette(c("yellow", "red"), alpha = 0.8)(length(ii))[ii]
# Change polar to long lat coordinates
#x <- ((x_90_polar*1000/110815) * sin(wd_polar_rad)) + 100.2025
#y <- ((x_90_polar*1000/110584) * cos(wd_polar_rad)) + 5.49
...
#points(100.2025, 5.4750, pch = 19, col = 'black', cex = 1)
#scaleBathy(bath_ppinang, deg = 0.045, x = 'bottomright', inset = 5, cex = 0.8)
#points(x, y, pch = 19, col = colors, cex = 2)

#legend_number <- sort(levels(ii), decreasing = TRUE)
#legend('topleft', 
#       legend = c(legend_number[1], legend_number[3], legend_number[4], legend_number[6]),
#       pch = rep(19,length(levels(as.factor(colors)))),
#       col = c(levels(as.factor(colors))), cex = 1)
